...
async function handler(req, res) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      const { arrayOfIds, arrayOfIndicators } = req.body;
      const table_name = [
        ...new Set(arrayOfIndicators.map((x) => x.table_name)),
      ][0];
      const indicators2 = await db.query(
        'select * from $1:name where hex08_key in ($2:csv)',
        [table_name, arrayOfIds],
      );
